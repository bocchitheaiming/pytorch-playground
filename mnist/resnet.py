...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        m = OrderedDict()
        # 1*64*28*28
        m['conv1'] = nn.Conv2d(1, 64, kernel_size=5, stride=1, padding=2, bias=False) # 1*64*28*28
        ...
        # 残差块
        self.layer1 = self._make_layer(block, 64, layers[0]) # 1*64*28*28
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2) # 1*128*14*14
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2) # 1*256*7*7
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2) # 1*512*3*3

        self.avgpool = nn.Sequential(nn.AvgPool2d(2)) # 1*512*1*1
